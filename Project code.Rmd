...
```{r model comparison}
# null model
model_null <- lmer(distance ~ 1 + (1 | ID), data = data_long, REML = FALSE)
# model without controls
model_without_controls <- lmer(distance ~ riskHI * trauma_lifetime + (1|ID), data = data_long, REML = FALSE)
# fixed effect
model_fixed <- lmer(distance ~ riskHI + trauma_lifetime + financial_hardship + SES + (1 | ID), data = data_long, REML = FALSE)
...
# comparison
anova(model_null, model_without_controls)
anova(model_fixed, model_interaction, model_random_slopes)

# ICC
icc(model_null)
icc(model_fixed)
icc(model_without_controls)
